(*
	...
	This script takes the currently selected actions or projects and sets them for action on
	Monday.
	(If Monday is currently in progress, the items will be set for the *current* Monday.)
...
property weEndDay : Monday
property weEndTime : 17 --due time in hours (24 hr clock)
property weStartDay : Monday
property weStartTime : 6 --due time in hrs (24 hr clock)
...
on main()
	...
	if showSummaryNotification then
		...
		set alertText to successTot & " item" & alertItemNum & " now starting on Monday." as string
		my notify("General", "Monday Script complete", alertText)
	end if
end main
